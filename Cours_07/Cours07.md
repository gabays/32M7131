...
# Les modèles de langue et les champs lexicaux
...
Approche multidisciplinaire (linguistique, informatique…) dont les objets sont très proches de la philologie computationnelle

Le TAL a pour but de créer des outils de traitement de la langue naturelle pour diverses applications. On parle de tâche: la lemmatisation, l'annotation des parties du discours…

---

## Trois grands objectifs:

1. Analyser: extraction d'information, analyse de sentiment, fouille de texte…
2. Génération: génération de dialogue (pour les chatbots), journalisme automatique…
3. transformation: traduction automatique, résumé automatique, simplification de texte…

---
## Nouveau paradigme du TAL

Le passage à des approches neuronales a deux implications fortes:
1. L'apparition des modèles de langue
2. Le besoin de corpus bruts massifs pour entraîner ces modèles

Actuellement on entend beaucoup par de modèles BERT –– reconnaissable avec leur nom qui finit en "-BERT" comme _CamemBERT_ pour le français contemporain, ou _d'AlemBERT_ pour le français d'Ancien Régime (préclassique et classique).

Ces modèles s'appuient sur des corpus: OSCAR pour _CamemBERT_ et FreEM max pour _d'AlemBERT_.

Cf. https://freem-corpora.github.io/corpora/max/
...
On parle d'"hypothèse distributionnelle" : deux mots sont similaires s’ils apparaissent dans les mêmes contextes. Ainsi "médecin" et "doceur" apparaissent dans des contextes similaires.
...
Word2vec n'est qu'un _lookup table_.

![50% center](images/word2vec.png)
...
![45% center](images/CBOW_train.png)
...
* _Glove_ va pondérer la prédiction avec les statistiques de fréquence à l'échelle du corpus. Il construit une matrice de co-occurence (le nombre de mots par le nombre de context possible):
...
---
## Les _embeddings_ contextuels: modèle génératif

![25% center](images/decodeur.png)

Modèle de type GPT3

---
## Les _embeddings_ contextuels: modèle adirectionnel

![25% center](images/encodeur.png)
...
![45% center](images/graph.png)

---

## Soutien à des tâches de TAL: fournir des _embeddings_

![55% center](images/fournisseur_emb.png)

---

## Soutien à des tâches de TAL: fournir des _fine-tuning_

![55% center](images/fine-tune.png)

On parle donc d'entraînement semi-supervisé

---
## Le transformeur

![45% center](images/transformeur.png)

L'exemple parfait est celui de la traduction, qui nécessite d'analyser la phrase source, et de générer la phrase cible.

-> Le mécanisme d'attention permet de ne pas générer _mot à mot_.

---

## Mécanisme d'attention

![55% center](images/seg2seq.JPG)

![55% center](images/attention.JPG)

---

Ce cours s'inspire fortement de celui de B. Sagot donné à Genève en décemnre 2022 dans le cadre d'un atelier PDEN